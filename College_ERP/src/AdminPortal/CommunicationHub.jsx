...
export default function CommunicationHub() {
  ...
  // Chart Data as State with Fallback
      ...
      {activeTab === "analytics" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-[var(--card)] dark:bg-[var(--card)] p-4 rounded-lg shadow">
            ...
            {readRateData && readRateData.labels && (
              <Bar data={readRateData} options={{ plugins: { zoom: { zoom: { mode: 'x' } } } }} />
            )}
          </div>
          <div className="bg-[var(--card)] dark:bg-[var(--card)] p-4 rounded-lg shadow">
            <h3 className="text-lg font-medium mb-4 flex items-center" style={{ color: `var(--text)` }}>
            ...
            {notificationTypeDistributionData && notificationTypeDistributionData.labels && (
              <Pie data={notificationTypeDistributionData} options={{ plugins: { zoom: { zoom: { mode: 'xy' } } } }} />
            )}
          </div>
        </div>
      )}
